...
class Renderer:
    ...
    def _place_character(
        ...
        #    - ความกว้าง: แยกระหว่างตัวกลางกับตัวข้าง
        # 🎯 ใช้ค่าจาก config (4.2 = เข็มขัด, 3.5 = เอว) - ปรับใน config.py เพื่อ revert ได้ง่าย!
        crop_height = int(face_h * settings.CHARACTER_CROP_HEIGHT_MULTIPLIER)

        # เช็คว่าเป็นตัวกลางหรือไม่ (ตำแหน่ง x ≈ 50% ของความกว้างจอ)
        is_center = abs(placement.position.x - self.width * 0.50) < 50  # ห่างจากกลางไม่เกิน 50px

        if is_center:
            # ตัวกลาง: crop แคบ (เอาแค่ตัวคน ไม่มีพื้นที่ข้างๆ)
            crop_width = int(face_h * 1.8)  # แคบ 1.8x เพื่อไม่ให้มีขอบขาว
        else:
            # ตัวข้าง: crop พอดี (ลดจาก 3.2x เป็น 2.4x เพื่อไม่ให้ทับกัน)
            crop_width = int(face_h * 2.4)  # 2.4x เพื่อไม่ให้กว้างเกินไปและทับกัน
        ...
        # ======================================================================
        # เป้าหมาย: ปรับขนาดโดยไม่ยืดรูป (รักษาสัดส่วนเดิม)
        # วิธีการ: คำนวณ target height จาก scale แล้วคำนวณ width ตาม aspect ratio
        # ======================================================================

        # 1. คำนวณความสูงเป้าหมายจาก scale parameter
        #    scale = 1.1 หมายถึง 110% ของความสูง canvas
        target_h = int(self.height * placement.scale)

        # 2. คำนวณความกว้างให้สอดคล้องกับ aspect ratio เดิม
        #    ป้องกันการยืดหรือบีบรูป
        aspect_ratio = character_img.width / character_img.height
        target_w = int(target_h * aspect_ratio)

        # 3. Resize โดยรักษา aspect ratio และใช้ LANCZOS สำหรับคุณภาพสูง
